...
+ (NSString*)filePathForRequestKey:(NSString*)url username:(NSString*)username {
    if(username != nil && url != nil) {
        ...
        {
            NSString* audiosPath = [userPath stringByAppendingPathComponent:@"Audios"];
            
            if([[NSFileManager defaultManager] fileExistsAtPath:audiosPath]) {
                BOOL success = [[NSFileManager defaultManager] removeItemAtPath:audiosPath error:&error];
                if (!success) {
...
+ (NSString*)filePathForAudioURL:(NSString*)audioUrl username:(nullable NSString *)username {
    NSString* filepath = [[OEXFileUtility filePathForRequestKey:audioUrl username:username] stringByAppendingPathExtension:@"mp3"];
    return filepath;
}
...
+ (BOOL) canDeleteFile:(NSString *) file {
    ...
    if ([fileExtension isEqualToString:@"mp4"] || [fileExtension containsString:@"sqlite"] || [fileExtension containsString:@"mp3"] ) {
        return NO;
    }
