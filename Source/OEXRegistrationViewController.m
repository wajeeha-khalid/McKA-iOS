...
@property (strong, nonatomic) UILabel* fieldsDescriptionLabel;
...
@property (strong, nonatomic) SpinnerView* progressIndicator;
...
- (void)initializeViews {
    ...
    self.logoImageView.image = [UIImage imageNamed:@"pique_logo"];
    [self.scrollView addSubview:self.logoImageView];

    ////Create and initalize 'btnCreateAccount' button
    self.registerButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.registerButton.adjustsImageWhenHighlighted = YES;
    
    [self.registerButton oex_addAction:^(id  _Nonnull control) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.registerButton applyButtonStyle:[self.environment.styles filledButtonStyle:[UIColor piqueGreen]] withTitle:[Strings registrationCreateMyAccount]];
        });
    ...
    [self.registerButton oex_addAction:^(id  _Nonnull control) {
        [self.registerButton applyButtonStyle:[self.environment.styles filledButtonStyle:[[UIColor alloc] initWithHexString:@"4b856d" alpha:1.0]] withTitle:[Strings registrationCreateMyAccount]];
    ...
    self.progressIndicator = [[SpinnerView alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
    ...
    self.fieldsDescriptionLabel = [[UILabel alloc] init];
    self.fieldsDescriptionLabel.font = [UIFont systemFontOfSize:12 weight:UIFontWeightMedium];
    self.fieldsDescriptionLabel.isAccessibilityElement = NO;
    self.fieldsDescriptionLabel.text = @"*All fields are mandatory";
    self.fieldsDescriptionLabel.textColor = [[UIColor alloc] initWithHexString:@"#626567" alpha:0.5];
    [self.scrollView addSubview:self.fieldsDescriptionLabel];
    ...
    [self.agreementLink setTitleColor:[UIColor colorWithRed:0/255.0 green:221/255.0 blue:253/255.0 alpha:1.0] forState:UIControlStateNormal];
    ...
    self.toggleOptionalFieldsButton.hidden = YES;
...
- (IBAction)navigateBack:(id)sender {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Would you like to close the Sign Up Page" message:@"This will redirect you to the Sign In page. Do you want to continue?" preferredStyle:UIAlertControllerStyleAlert];

    [alert addAction:[UIAlertAction actionWithTitle:@"Don't Allow" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
- (void)viewDidLayoutSubviews {
    ...
    [self.fieldsDescriptionLabel setFrame:CGRectMake(25, offset, self.scrollView.frame.size.width - 25*2, 21)];

    offset += 63;

    [self.registerButton setFrame:CGRectMake(9, offset, self.scrollView.frame.size.width - 9*2, 50)];
    ...
    NSInteger buttonLabelSpacing = 15;
