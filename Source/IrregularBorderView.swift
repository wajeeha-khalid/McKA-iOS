...
class IrregularBorderView : UIImageView {
    ...
    var style : IrregularBorderStyle? {
        didSet {
            if let style = style {
                ...
                self.cornerMaskView.image = renderMaskWithCorners(style.corners, cornerRadii: CGSize(width: radius, height: radius))
    ...
    fileprivate func renderMaskWithCorners(_ corners : UIRectCorner, cornerRadii : CGSize) -> UIImage {
    ...
    fileprivate func renderBorderWithEdges(_ corners : UIRectCorner, style : BorderStyle) -> UIImage? {
