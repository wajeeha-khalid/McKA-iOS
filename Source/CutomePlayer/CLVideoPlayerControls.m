...
//@property (nonatomic, strong) UIView* activityBackgroundView; //Commented by Ravi on 21Feb'17 to make the variable as public to use in AudioPodcast Controller
...
@property (nonatomic, assign) BOOL isAudioPlayer; // Added By Ravi on 21 Feb'17 for AudioPodcast settings Control
...
@property (nonatomic, assign) BOOL shouldSendProgressNotification;
...
- (id)initWithMoviePlayer:(CLVideoPlayer*)moviePlayer style:(CLVideoPlayerControlsStyle)style {
    self = [super init];
    if(self) {
        self.shouldSendProgressNotification = YES;
        self.seeking = NO;
        ...
        _hideNext = YES;
        _hidePrevious = YES;
         _isAudioPlayer = NO; // Added By Ravi on 21 Feb'17 for AudioPodcast settings Control
...
- (void)dealloc {
    ...
    [self nilDelegates];
    _durationSlider = nil;
    _shouldSendProgressNotification = NO;
}
...
- (void)setup {
    ...
    [_playPauseButton setImage:[UIImage imageNamed:@"play_icon"] forState:UIControlStateSelected];
    ...
    //[_bottomBar addSubview:_btnSettings]; // Commented by Ravi on 10Mar2016 as settings button is not required.
...
- (void)setHidesFullSceenButton:(BOOL)hidesFullSceenButton {
    _hidesFullSceenButton = hidesFullSceenButton;
    self.fullscreenButton.hidden = hidesFullSceenButton;
}
...
- (void)playPausePressed:(UIButton*)button {
	...
	self.moviePlayer.userPlayRequested = YES;
...
- (void)moviePlaybackStateDidChange:(NSNotification*)note {
    ...
    //Added By Ravi on 21Feb'17  for AudioPOcast settings tableview.
    CLVideoPlayer *audioPlayer = weakSelf.moviePlayer;
    
    if([[audioPlayer.contentURL absoluteString] containsString:@".mp3"])
    {
        _isAudioPlayer = YES;
    }
            ...
            if(weakSelf.moviePlayer.loadState == MPMovieLoadStatePlaythroughOK ||
               weakSelf.moviePlayer.loadState == MPMovieLoadStateUnknown) {
                if(weakSelf.stateBeforeSeek == MPMoviePlaybackStatePlaying && !self.seeking) {
                    ...
                    CLVideoPlayer *player = weakSelf.moviePlayer;
                    if(![[player.contentURL absoluteString] containsString:@".mp3"])
                    {
...
- (void)layoutSubviews {
    ...
    CGFloat settingsbtnSize = 0.f;
    CGFloat tableOptionWidth = 120.f;
    CGFloat tableOptionHeight = 88.f;
    
     //Added By Ravi on 21Feb'17  for AudioPOcast settings tableview.
    if (_isAudioPlayer == YES)
    {
        tableOptionHeight = 44.f;
    }
    ...
    CGFloat fullscreenBtnSize = self.fullscreenButton.hidden ? 0.f : 20.f;
    ...
    if(self.style == CLVideoPlayerControlsStyleFullscreen || (self.style == CLVideoPlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        self.btnSettings.frame = CGRectMake(self.fullscreenButton.frame.origin.x - settingsbtnSize, self.barHeight / 2 - settingsbtnSize / 2, settingsbtnSize, settingsbtnSize);
