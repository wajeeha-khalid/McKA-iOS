...
public class NetworkManager : NSObject {
    ...
    public func updateCourseProgress(userName:String, componentIDs:String, onCompletion: ServiceResponse) -> Void {
        let request = NSMutableURLRequest(URL: NSURL(string:"\(self.baseURL)/"+"api/progress_tracker/recordView/")!)
        let requestBody = ["userName": userName, "componentIds": componentIDs]
        request.HTTPMethod = HTTPMethod.POST.rawValue
        do{
        let requestData  = try NSJSONSerialization.dataWithJSONObject(requestBody, options: NSJSONWritingOptions.PrettyPrinted)
