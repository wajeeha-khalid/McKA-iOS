...
open class Stream<A> : StreamDependency {
    ...
    open func firstSuccess() -> edXCore.Stream<A> {
    ...
    open func dropFailuresAfterSuccess() -> edXCore.Stream<A> {
        let sink = Sink<A>(dependencies: [self])
        listen(sink.token) {[weak sink] result in
    ...
    open func map<B>(_ f : @escaping (A) -> B) -> edXCore.Stream<B> {
        return resultMap { $0.map(f) }
    }
    
    /// Transforms a stream into a new stream.
    open func resultMap<B>(_ f : @escaping (Result<A>) -> Result<B>) -> edXCore.Stream<B> {
    ...
    /// Transforms a stream into a new stream.
    open func flatMap<B>(fireIfAlreadyLoaded: Bool = true, f : @escaping (A) -> Result<B>) -> edXCore.Stream<B> {
        let sink = Sink<B>(dependencies: [self])
    ...
    open func transform<B>(_ f : @escaping (A) -> edXCore.Stream<B>) -> edXCore.Stream<B> {
    ...
    open func autoCancel(_ cancelAction : Removable) -> edXCore.Stream<A> {
    ...
    open func cachedByStream(_ cacheStream : edXCore.Stream<A>) -> edXCore.Stream<A> {
    ...
    open func delay(_ duration : TimeInterval) -> edXCore.Stream<A> {
...
open class Sink<A> : edXCore.Stream<A> {
...
open class BackedStream<A> : edXCore.Stream<A> {
    ...
    @discardableResult open func backWithStream(_ stream : edXCore.Stream<A>) -> Removable {
        removeAllBackings()
        return addBackingStream(stream)
    }
    
    @discardableResult open func addBackingStream(_ stream : edXCore.Stream<A>) -> Removable {
...
public func joinStreams<T, U>(_ t : edXCore.Stream<T>, _ u: edXCore.Stream<U>) -> edXCore.Stream<(T, U)> {
...
public func joinStreams<T>(_ streams : [edXCore.Stream<T>]) -> edXCore.Stream<[T]> {
    ...
    let pairs = streams.map {(stream : edXCore.Stream<T>) -> (box : MutableBox<Resolution<T>>, stream : edXCore.Stream<T>) in
...
public func accumulate<A>(_ source : edXCore.Stream<[A]>) -> edXCore.Stream<[A]> {
