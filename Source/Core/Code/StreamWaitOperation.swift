...
class StreamWaitOperation<A> : edXCore.Operation {
    fileprivate let completion : ((Result<A>) -> Void)?
    fileprivate let stream : edXCore.Stream<A>
    fileprivate let fireIfAlreadyLoaded : Bool

    init(stream : edXCore.Stream<A>, fireIfAlreadyLoaded : Bool, completion : ((Result<A>) -> Void)? = nil) {
