        ...
        @synchronized(_masterManagedObjectContext){
            ...
            if(_masterManagedObjectContext != nil) {
                if([_masterManagedObjectContext hasChanges] && ![_masterManagedObjectContext save:&error]) {
                    ...
                    [[NSNotificationCenter defaultCenter] postNotificationName:kLocalSavingNotification object:nil];
                }
            }
        }
    }];
...
- (void)markPlayedState:(OEXPlayedState)state forAudioID:(NSString*)audio_id {
    AudioData* audioData = [self getAudioDataForAudioID:audio_id];

    if(audioData) {
        audioData.played_state = [NSNumber numberWithFloat: state];
        [self saveCurrentStateToDB];
    }
}
...
- (void)deleteFileForURL:(NSString*)URL {
    ...
    {
        [[NSFileManager defaultManager] removeItemAtPath:[OEXFileUtility filePathForAudioURL:URL username:[OEXSession sharedSession].currentUser.username]
                                                   error:nil];
...
- (NSArray*)getVideosForDownloadUrl:(NSString*)downloadUrl {
   ...
   // NSPredicate* query = [NSPredicate predicateWithFormat:@"video_url==%@", downloadUrl]; Commented by Ravi on 20Jan'17 to Implement VRVideo Download.
    NSPredicate *query = [NSPredicate predicateWithFormat:@"video_url CONTAINS[cd] %@",
                              downloadUrl];  // Added By Ravi on 20Jan'17 as the URL is modified to Download VRVideo file.
    [fetchRequest setPredicate:query];
...
//Added By Ravi on 30Jan'17 fir Audio Download Podcast
#pragma  mark - AudioData Table Methods

- (OEXPlayedState)watchedStateForAudioID:(NSString*)audio_id {
    AudioData* audioData = [self getAudioDataForAudioID:audio_id];

    if(audioData) {
        return [audioData.played_state intValue];
    }
    return OEXPlayedStateUnwatched;
}

- (AudioData*)audioDataForAudioID:(NSString*)audio_id {
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* videoEntity = [NSEntityDescription entityForName:@"AudioData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:videoEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"audio_id==%@", audio_id];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
    NSArray* resultArray = [self executeFetchRequest:fetchRequest];
    //   ELog(@"getVideoDataFor VideoID : %@",resultArray);
    
    return (AudioData*)[resultArray firstObject];
}

//Added by Naveen

- (ComponentData *)componentDataforComponentID:(NSString *)component_id {
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* componentEntity = [NSEntityDescription entityForName:@"ComponentData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:componentEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"componentID == %@", component_id];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
    NSArray* resultArray = [self executeFetchRequest:fetchRequest];
    //   ELog(@"getVideoDataFor VideoID : %@",resultArray);
    
    return (ComponentData*)[resultArray firstObject];
}

- (UnitData *)unitDataForUnitID:(NSString *)sequential_id {
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* untiEntity = [NSEntityDescription entityForName:@"UnitData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:untiEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"unitID == %@", sequential_id];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
    NSArray* resultArray = [self executeFetchRequest:fetchRequest];
    //   ELog(@"getVideoDataFor VideoID : %@",resultArray);
    
    return (UnitData*)[resultArray firstObject];
}

- (NSArray *)getViewedComponentsForVerticalID:(NSString *)sequential_id {
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* videoEntity = [NSEntityDescription entityForName:@"ComponentData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:videoEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"unitID==%@ && isViewed == %d", sequential_id, true];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
    NSArray* resultArray = [self executeFetchRequest:fetchRequest];
    //   ELog(@"getVideoDataFor VideoID : %@",resultArray);
...
- (NSArray *)getViewedComponentsForCourseId:(NSString *)course_id {
    ...
    NSEntityDescription* videoEntity = [NSEntityDescription entityForName:@"ComponentData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:videoEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"courseID == %@ && isViewed == %d", course_id, true];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
    NSArray* resultArray = [self executeFetchRequest:fetchRequest];
...
- (NSMutableArray *)getUnsyncedComponents{
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* videoEntity = [NSEntityDescription entityForName:@"ComponentData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:videoEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"synced == %@ && isViewed == %d", false, true];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
...
-  (NSArray *)getCompletedUnitsListForChapterID:(NSString *)chapter_id{
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* unitEntity = [NSEntityDescription entityForName:@"UnitData" inManagedObjectContext:self.backGroundContext];
    [fetchRequest setEntity:unitEntity];
    NSPredicate* query = [NSPredicate predicateWithFormat:@"chapterID == %@", chapter_id];
    //setting the predicate to the fetch request
    [fetchRequest setPredicate:query];
    NSArray* resultArray = [self executeFetchRequest:fetchRequest];
    //   ELog(@"getVideoDataFor VideoID : %@",resultArray);
...
//inserting the audio data only if the video is played online or started downloading.

//Added by Naveen to store the Unit data and component data
- (ComponentData*)insertComponentData:(NSString*)title
                          viewedState:(BOOL)viewed_state
                               unitID:(NSString*)unitID
                          componentID:(NSString*)componentID
                             courseID:(NSString*)courseID {
    
    ComponentData* checkdata = [self getComponentDataForComponentID:componentID];
    
    if(checkdata != nil) {
        return checkdata;
    }
    
    ComponentData* componentObj = [NSEntityDescription insertNewObjectForEntityForName:@"ComponentData"
                                                                inManagedObjectContext:_backGroundContext];
    
    //videoObj.username = username;
    componentObj.unitID = unitID;
    componentObj.componentID = componentID;
    componentObj.courseID = courseID;
    componentObj.isViewed = [NSNumber numberWithBool:viewed_state];
    
    [self saveViewedUnitToDB];
    
    return componentObj;
}

- (void)updateComponentViewedSync:(NSString*)componentID withSynced:(BOOL)synced{
    ComponentData *componentObj = [self getComponentDataForComponentID:componentID];
    componentObj.synced = [NSNumber numberWithBool:synced];
    [self saveCurrentStateToDB];
}

- (UnitData*) insertUnitDataForID:(NSString*)unitID
                         courseID:(NSString *)courseID
                        chapterID:(NSString *)chapterID
                   completedState:(BOOL)completed {
    
    UnitData* checkdata = [self getUnitDataForUnitID:unitID];
    
    if(checkdata != nil) {
        return checkdata;
    }
    
    UnitData* unitObj = [NSEntityDescription insertNewObjectForEntityForName:@"UnitData"
                                                      inManagedObjectContext:_backGroundContext];
    
    unitObj.isCompleted = [NSNumber numberWithBool:completed];
    unitObj.unitID = unitID;
    unitObj.courseID = courseID;
    unitObj.chapterID = chapterID;
...
- (AudioData*)insertAudioData:(NSString*)username
                        Title:(NSString*)title
                         Size:(NSString*)size
                     Duration:(NSString*)duration
                DownloadState:(OEXDownloadState)download_state
                     AudioURL:(NSString*)audio_url
                     AudioID:(NSString*)audio_id
    ...
    AudioData* Checkdata = [self getAudioDataForAudioID:audio_id];
    
    if(Checkdata && audio_id != nil) {
        return Checkdata;
    }
    
    AudioData* audioObj = [NSEntityDescription insertNewObjectForEntityForName:@"AudioData"
                                                        inManagedObjectContext:_backGroundContext];
    ...
    audioObj.download_state = [NSNumber numberWithInt:download_state];
    audioObj.audio_url = audio_url;
    audioObj.audio_id = audio_id;
    audioObj.chapter_name = chapterName;
    audioObj.section_name = sectionName;
    audioObj.played_state = [NSNumber numberWithInt:played_state];
    audioObj.course_id = course_id;
    audioObj.downloadCompleteDate = downloadCompleteDate;
...
- (AudioData*)getAudioDataForAudioID:(NSString*)audioId {
    return [self audioDataForAudioID:audioId];
}

// Added by Naveen

- (ComponentData *)getComponentDataForComponentID:(NSString*)componentId{
    return [self componentDataforComponentID:componentId];
}

- (UnitData *)getUnitDataForUnitID:(NSString*)sequential_id{
    return [self unitDataForUnitID:sequential_id];
}

- (NSArray*)getAllDownloadingAudiosForURL:(NSString*)url {
    NSArray* allAudios = [self getAudiosForDownloadUrl:url];
    NSMutableArray* filteredArray = [[NSMutableArray alloc] init];
    for(AudioData* data in allAudios) {
        if([data.download_state intValue] == OEXDownloadStatePartial) {
            [filteredArray addObject:data];
        }
    }
    return filteredArray;
...
- (NSArray*)getAudiosForDownloadUrl:(NSString*)downloadUrl {
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    
    [fetchRequest setEntity:[NSEntityDescription entityForName:@"AudioData" inManagedObjectContext:_backGroundContext]];
    
    NSPredicate* query = [NSPredicate predicateWithFormat:@"audio_url==%@", downloadUrl];
      [fetchRequest setPredicate:query];
...
- (void)completedDownloadForAudio:(AudioData*)audioData {
    if(audioData) {
        audioData.download_state = [NSNumber numberWithFloat:OEXDownloadStateComplete];
        audioData.downloadCompleteDate = [NSDate date];
        [self saveCurrentStateToDB];
    }
}

- (void)cancelledDownloadForAudio:(AudioData*)audioData {
    if(audioData) {
        audioData.download_state = [NSNumber numberWithFloat:OEXDownloadStateNew];
        [self deleteDataForAudioURL:audioData.audio_id];
        [self saveCurrentStateToDB];
    }
}

- (void)deleteDataForAudioURL:(NSString*)studentViewURL {
    
    AudioData* audioData = [self getAudioDataForAudioID:studentViewURL];
    NSArray* arrAudio = [self getAudiosForDownloadUrl:audioData.audio_url];
    
    int referenceCount = 0;
    if([arrAudio count] > 1) {
        for(AudioData* audio in arrAudio) {
            if([audio.download_state intValue] == OEXDownloadStateComplete) {
                referenceCount++;
                if(referenceCount >= 2) {
                    break;
                }
    ...
    if(audioData) {
        //Mark new in DB
        audioData.download_state = @(OEXDownloadStateNew);
        [self saveCurrentStateToDB];
        //Delete file
        if(referenceCount <= 1) {
            [self deleteFileForURL:audioData.audio_url];
        }
    }
}


- (NSArray*)getAudiosForDownloadState:(OEXDownloadState)state {
    NSArray* allAudios = [self getAllLocalAudioData];
    NSMutableArray* filteredArray = [[NSMutableArray alloc] init];
...
- (NSArray*)getAllLocalAudioData {
    NSFetchRequest* fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription* audioEntity = [NSEntityDescription entityForName:@"AudioData" inManagedObjectContext:_backGroundContext];
