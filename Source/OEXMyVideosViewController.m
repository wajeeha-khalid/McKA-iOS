...
@property (strong, nonatomic) NSMutableArray *courseArray; // Added By Ravi on 3Mar'17 to get list of all courses.
...
- (void)viewDidLoad {
    ...
    //PiQUE : Hiding collection view
    [self.tabView setHidden:YES];
    self.tabViewHeight.constant = 0;
...
- (void)getMyVideosTableData {
    ...
    //Added By Ravi on 12Feb2017 for Audio Podcast
    // Populate both ALL & RECENT Audios Table data
    for(NSDictionary* dict in arrCourseAndAudio) {
        NSMutableDictionary* mutableDict = [dict mutableCopy];
        
        NSString* strSize = [[NSString alloc] initWithString: [self calculateAudiosSizeInCourse:[mutableDict objectForKey:CAV_KEY_AUDIOS]] ];
        NSMutableArray* sortedArray = [mutableDict objectForKey:CAV_KEY_AUDIOS];
        NSSortDescriptor* sortDesc = [[NSSortDescriptor alloc] initWithKey:@"completedDate" ascending:NO selector:@selector(compare:)];
        [sortedArray sortUsingDescriptors:[NSArray arrayWithObject:sortDesc]];
        NSMutableArray* arr_SortedArray = [sortedArray mutableCopy];
        NSDictionary* audios = @{CAV_KEY_COURSE: [mutableDict objectForKey:CAV_KEY_COURSE],
                                 CAV_KEY_AUDIOS: [mutableDict objectForKey:CAV_KEY_AUDIOS],
                                 CAV_KEY_RECENT_AUDIOS: arr_SortedArray,
                                 CAV_KEY_AUDIOS_SIZE: strSize};
        
        [self.arr_CourseData addObject:audios];
        
    }

    
    
    // Added by Ravi on 4Mar'17 to find the exact course count in NumberofsectionsinTableview.
    self.courseArray = [[NSMutableArray alloc] init];
    
    if([self.arr_CourseData count] > 0)
    {
        NSArray *tempCourseArray = [self.arr_CourseData valueForKey:CAV_KEY_COURSE];
        NSString *courseId = @"";
        
        for (OEXCourse *course in tempCourseArray)
        {
            if([courseId length] == 0)
            {
                 courseId = [course course_id]; // assign the Course id
                [self.courseArray addObject:course];
            }
           else if (![courseId isEqualToString:[course course_id]] && ![self.courseArray containsObject:course])
           {
               courseId =  [course course_id]; // Update the Course id
               [self.courseArray addObject:course];
           }
        }
...
- (NSString*)calculateAudiosSizeInCourse:(NSArray*)arraudio {
    NSString* strSize = nil;
    
    double size = 0.0;
    
    for(OEXHelperAudioDownload* audio in arraudio) {
        double audioSize = [audio.summary.size doubleValue];
        double sizeInMegabytes = (audioSize / 1024) / 1024;
        size += sizeInMegabytes;
    }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView*)tableView {
    // Added by Ravi on 4Mar'17 to show exact course count
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath {
    if(tableView == self.table_MyVideos) {
        ...
        infoView.isMyVideos = true;
        [infoView.titleAccessoryView setHidden:NO];
