...
func shareTextAndALink(_ text: String, url: URL, analyticsCallback:((String) -> Void)?) -> UIActivityViewController {
    let items = [text, url] as [Any]
    return controllerWithItems(items as [AnyObject], analyticsCallback: analyticsCallback)
}

func shareHashtaggedTextAndALink(_ textBuilder: @escaping (_ hashtagOrPlatform: String) -> String, url: URL, analyticsCallback:((String) -> Void)?) -> UIActivityViewController {
    let items = [PlatformHashTag(textBuilder: textBuilder), url] as [Any]
    return controllerWithItems(items as [AnyObject], analyticsCallback: analyticsCallback)
}

private func controllerWithItems(_ items: [AnyObject], analyticsCallback:((String) -> Void)?) -> UIActivityViewController{
