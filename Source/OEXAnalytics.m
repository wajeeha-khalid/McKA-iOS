...
@interface OEXAnalyticsAudioEvent : OEXAnalyticsEvent

@property (copy, nonatomic) NSString* audioModuleID;

@end

@implementation OEXAnalyticsAudioEvent
@end
...
// Added By Ravi on 31Jan'17 for Audio Podcast download
#pragma mark- Audio Podcast


- (void)trackSingleAudioDownload:(NSString*)audioID
                        CourseID:(NSString*)courseId
                         UnitURL:(NSString*)unitUrl {
    NSMutableDictionary* info = @{}.mutableCopy;
    
    OEXAnalyticsAudioEvent* event = [[OEXAnalyticsAudioEvent alloc] init];
    event.name = audio_value_single_download;
    event.displayName = @"Single Audio Download";
    event.courseID = courseId;
    event.openInBrowserURL = unitUrl;
    event.audioModuleID = audioID;
    
    [self trackAudioDownloadEvent:event withInfo:info];
}


- (void)trackAudioDownloadEvent:(OEXAnalyticsAudioEvent*)event withInfo:(NSDictionary*)info {
    [self trackAudioEvent:event forComponent:value_downloadmodule withInfo:info];
}


- (void)trackAudioEvent:(OEXAnalyticsAudioEvent*)event forComponent:(NSString*)component withInfo:(NSDictionary*)info {
    NSMutableDictionary* fullInfo = [[NSMutableDictionary alloc] initWithDictionary:info];
    [fullInfo setObjectOrNil:event.audioModuleID forKey:key_module_id];
    [fullInfo setObjectOrNil:value_mobile forKey:key_code];
    [self trackEvent:event forComponent:component withInfo:fullInfo];
}

- (void)trackAudioDownloadComplete:(NSString*)audioId
                     CourseID:(NSString*)courseId
                      UnitURL:(NSString*)unitUrl {
    NSMutableDictionary* info = @{}.mutableCopy;
    
    OEXAnalyticsAudioEvent* event = [[OEXAnalyticsAudioEvent alloc] init];
    event.name = value_audio_downloaded;
    event.displayName = @"Audio Downloaded";
    event.courseID = courseId;
    event.openInBrowserURL = unitUrl;
    event.audioModuleID = audioId;
